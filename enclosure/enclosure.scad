...
module box()
{
    difference()
    {
        translate([ -WALL_THICKNESS, -WALL_THICKNESS, -WALL_THICKNESS ]) cube([
            INTERNAL_WIDTH + 2 * WALL_THICKNESS, INTERNAL_DEPTH + 2 * WALL_THICKNESS, INTERNAL_HEIGHT + WALL_THICKNESS
        ]);
    ...
    // Corners
    translate([ CORNER_INSET_SIZE / 2, CORNER_INSET_SIZE / 2, 0 ]) difference()
    {
        cylinder(d = 2 * WALL_THICKNESS + CORNER_INSET_SIZE, h = INTERNAL_HEIGHT, $fn = 32);
        cylinder(d = CORNER_INSET_SIZE, h = INTERNAL_HEIGHT + 1, $fn = 32);
    }

    translate([ INTERNAL_WIDTH - CORNER_INSET_SIZE / 2, CORNER_INSET_SIZE / 2, 0 ]) difference()
    {
        cylinder(d = 2 * WALL_THICKNESS + CORNER_INSET_SIZE, h = INTERNAL_HEIGHT, $fn = 32);
        cylinder(d = CORNER_INSET_SIZE, h = INTERNAL_HEIGHT + 1, $fn = 32);
    }

    translate([ CORNER_INSET_SIZE / 2, INTERNAL_DEPTH - CORNER_INSET_SIZE / 2, 0 ]) difference()
    {
        cylinder(d = 2 * WALL_THICKNESS + CORNER_INSET_SIZE, h = INTERNAL_HEIGHT, $fn = 32);
        cylinder(d = CORNER_INSET_SIZE, h = INTERNAL_HEIGHT + 1, $fn = 32);
    }

    translate([ INTERNAL_WIDTH - CORNER_INSET_SIZE / 2, INTERNAL_DEPTH - CORNER_INSET_SIZE / 2, 0 ]) difference()
    {
        cylinder(d = 2 * WALL_THICKNESS + CORNER_INSET_SIZE, h = INTERNAL_HEIGHT, $fn = 32);
        cylinder(d = CORNER_INSET_SIZE, h = INTERNAL_HEIGHT + 1, $fn = 32);
    }
...
module lid()
{
    difference()
    {
        translate([ -WALL_THICKNESS, -WALL_THICKNESS, 0 ])
            cube([ INTERNAL_WIDTH + 2 * WALL_THICKNESS, INTERNAL_DEPTH + 2 * WALL_THICKNESS, WALL_THICKNESS ]);
